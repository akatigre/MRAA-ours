...
class Transform:
    ...
    def warp_coordinates(self, coordinates): # Thin-Plate-Spline Transformation
        ...
        if (self.loss_weights['equivariance_shift'] + self.loss_weights['equivariance_affine']) != 0:
            ...
            if self.loss_weights['equivariance_shift'] != 0:
                value = torch.abs(driving_region_params['shift'] -
                                  transform.warp_coordinates(transformed_region_params['shift'])) # batch_size,n_regions,2,2
                value = value.mean()
            ...
            if self.loss_weights['equivariance_affine'] != 0:
                affine_transformed = torch.matmul(transform.jacobian(transformed_region_params['shift']),
                                                  transformed_region_params['affine']) # # batch_size,n_regions,2,2

                normed_driving = torch.inverse(driving_region_params['affine']) # batch_size,n_regions,2,2
                normed_transformed = affine_transformed
                
                value = torch.matmul(normed_driving, normed_transformed) # batch_size,n_regions,2,2
