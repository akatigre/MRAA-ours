...
class SameBlock2d_DoubleConv(nn.Module):
    """
    Simple block, preserve spatial resolution.
    """

    def __init__(self, in_features, out_features, groups=1, kernel_size=3, padding=1):
        super(SameBlock2d_DoubleConv, self).__init__()
        self.conv = nn.Conv2d(in_channels=in_features, out_channels=out_features,
                              kernel_size=kernel_size, padding=padding, groups=groups)
        self.conv2 = nn.Conv2d(in_channels=out_features, out_channels=out_features,
                              kernel_size=kernel_size, padding=padding, groups=groups)
        self.norm1 = BatchNorm2d(in_features, affine=True)
        self.norm2 = BatchNorm2d(out_features, affine=True)
    def forward(self, source_feature, out_feature):
        out = torch.cat([source_feature, out_feature], dim=1)
        out = self.norm1(out)
        out = F.relu(out)
        out = self.conv(out)
        out = self.norm2(out)
