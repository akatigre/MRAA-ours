...
class Stage2_OF(nn.Module):
    ...
    def __init__(self, block_expansion, num_blocks, max_features, num_regions, num_channels,
                 scale_factor=1, region_var=0.01, use_deformed_source = False, 
                #estimate_occlusion_map=False, 
                 use_covar_heatmap=False, revert_axis_swap=False):
        super(Stage2_OF, self).__init__()
        ...
        self.mask = nn.Conv2d(self.hourglass.out_filters+2, num_regions + 1, kernel_size=(7, 7), padding=(3, 3))
    ...
    def forward(self, source_image, driving_region_params, source_region_params, stage1_dict, bg_params=None):
        """
        sparse motion: torch.Size([1, 11, 96, 96, 2])
        mask: torch.Size([1, 11, 96, 96])
        optical flow: torch.Size([1, 96, 96, 2])
        """
        if self.scale_factor != 1:
        ...
        prediction = torch.cat([prediction, F.interpolate(stage1_dict['optical_flow'].permute(0,3,1,2), prediction.shape[2:], mode='bilinear')], dim=1)
        ...
        out_dict['occlusion_map'] = stage1_dict['occlusion_map']
