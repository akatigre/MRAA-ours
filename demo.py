...
def make_animation(source_image, source_structure, driving_video, generator, region_predictor, avd_network,
                   animation_mode='standard', cpu=False):
    with torch.no_grad():
        predictions = []
        source_structure = torch.tensor(source_structure[np.newaxis].astype(np.float32)).permute(0, 3, 1, 2)
        source = torch.tensor(source_image[np.newaxis].astype(np.float32)).permute(0, 3, 1, 2)
        if not cpu:
            source = source.cuda()
            source_structure = source_structure.cuda()
        driving = torch.tensor(np.array(driving_video)[np.newaxis].astype(np.float32)).permute(0, 4, 1, 2, 3)
        source_region_params = region_predictor(source_structure)
        ...
        for frame_idx in tqdm(range(driving.shape[2])):
            ...
            new_region_params = get_animation_region_params(source_region_params, driving_region_params,
                                                            ...
                                                            mode=animation_mode)
            out = generator(source_structure, source, source_region_params=source_region_params, driving_region_params=new_region_params)
...
def main(opt):
    ...
    predictions = make_animation(source_image, source_structure, driving_video, generator, region_predictor, avd_network,
