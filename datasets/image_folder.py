...
def read_video(name, frame_shape):
    ...
    if os.path.isdir(name):
        ...
        video_array = [img_as_float32(io.imread(os.path.join(name, frames[idx]), plugin='matplotlib')) for idx in range(num_frames)]
...
class FramesDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        if self.is_train and os.path.isdir(path):
            frames = [frame for frame in os.listdir(path) if frame.endswith('.png')]
            num_frames = len(frames)
...
class LatticeDataset(Dataset):
    def __init__(self, root_dir, frame_shape=(256, 256, 3), id_sampling=False, is_train=True,
                 random_seed=0, pairs_list=None, augmentation_params=None):
        super().__init__()
        self.mgrid = self.get_2d_mgrid(frame_shape)
        self.root_dir = root_dir
        self.videos = os.listdir(root_dir)
        self.frame_shape = frame_shape
        self.pairs_list = pairs_list
...
class SRImplicitUniformVaried(Dataset):
    ...
    def __init__(self, dataset:LatticeDataset, lr_size, hr_size,
